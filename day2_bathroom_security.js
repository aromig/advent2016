// Advent of Code - Day 2
// http://adventofcode.com/2016/day/2

const input = ``;

const instructions = input.split('\n').map(x => x.split(''));

// Part 1

const keypad_p1 = '123/456/789'.split('/').map(x => x.split(''));
let pos = { x: 1, y: 1 } // start at '5'
let code = '';

function enterDigit_p1(line) {
    line.forEach(dir => {
        switch (dir) {
            case 'U':   pos.y -= pos.y > 0 ? 1 : 0;
                        break;
            case 'R':   pos.x += pos.x < 2 ? 1 : 0;
                        break;
            case 'D':   pos.y += pos.y < 2 ? 1 : 0;
                        break;
            case 'L':   pos.x -= pos.x > 0 ? 1 : 0;
                        break;
        }
    });
    
    return keypad_p1[pos.y][pos.x];
}

instructions.forEach(line => {
    code += enterDigit_p1(line);
});

console.log('Part 1:', code);

// Part 2

const keypad_p2 = '  1  / 234 /56789/ ABC /  D  '.split('/').map(x => x.split(''));
pos = { x: 0, y: 2 } // start at '5'
code = '';

function enterDigit_p2(line) {
    line.forEach(dir => {
        switch (dir) {
            case 'U':   if (pos.y > 0)
                            if (keypad_p2[pos.y-1][pos.x] != ' ')
                                pos.y--;
                        break;
            case 'R':   if (pos.x < 4)
                            if (keypad_p2[pos.y][pos.x+1] != ' ')
                                pos.x++;
                        break;
            case 'D':   if (pos.y < 4)
                            if (keypad_p2[pos.y+1][pos.x] != ' ')
                                pos.y++;
                        break;
            case 'L':   if (pos.x > 0)
                            if (keypad_p2[pos.y][pos.x-1] != ' ')
                                pos.x--;
                        break;
        }
    });
    
    return keypad_p2[pos.y][pos.x];
}

instructions.forEach(line => {
    code += enterDigit_p2(line);
});

console.log('Part 2:', code);